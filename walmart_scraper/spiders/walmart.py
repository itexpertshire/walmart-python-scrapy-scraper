...
class WalmartSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        for url in url_data.get("urls"):
            ...
            filter_words=url.get("filter_words")
            print(filter_words)
            walmart_search_url=url.get("url")+ urlencode(payload)
            yield scrapy.Request(url=walmart_search_url, callback=self.parse_search_results, meta={'keyword': keyword, 'page': 1, 'payload': payload, 'filter_words':filter_words})
    ...
    def parse_search_results(self, response):
        ...
        if script_tag is not None:
            ...
            for idx, product in enumerate(product_list):
                ...
                yield scrapy.Request(url=walmart_product_url, callback=self.parse_product_data,  meta={'keyword': keyword,'payload': payload,'filter_words':filter_words, 'page': page, 'position': idx + 1})
